// ─── Davomat routes ────────────────────────────────
// POST /api/davomat           — saqlash
// GET  /api/davomat           — ma'lumot olish
// GET  /api/davomat/tarix     — sanalar tarixi
// GET  /api/davomat/range     — davr bo'yicha
// POST /api/davomat/teacher   — o'qituvchi davomat saqlash
// GET  /api/davomat/teacher   — o'qituvchi davomat olish
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    await client.query('DELETE FROM davomat WHERE sana=$1 AND admin_username=$2', [p.sana, p.username]);
    for (const rec of records) {
      await client.query(
        `INSERT INTO davomat (sana,admin_username,sinf,oquvchi_ism,status,izoh,vaqt_belgilangan) VALUES ($1,$2,$3,$4,$5,$6,$7)`,
        [p.sana, p.username, rec.sinf, rec.ism, rec.status, rec.izoh||'', now]
      );
    }
...
router.get('/', async (req, res) => {
  const { username, parol, sana, adminUsername } = req.query;
  const admin = await verifyAdmin(username, parol);
  if (!admin) return res.status(401).json({ ok: false, error: "Ruxsat yo'q" });

  const target = (admin.isSuper && adminUsername) ? adminUsername : username;
...
router.get('/tarix', async (req, res) => {
  const { username, parol, adminUsername } = req.query;
  const admin = await verifyAdmin(username, parol);
  if (!admin) return res.status(401).json({ ok: false, error: "Ruxsat yo'q" });
...
router.get('/range', async (req, res) => {
  const { username, parol, from, to, adminUsername } = req.query;
  const admin = await verifyAdmin(username, parol);
  if (!admin) return res.status(401).json({ ok: false, error: "Ruxsat yo'q" });
  ...
  const filtered = result.rows.filter(r => {
    const d = parseUZ(r.sana); return d >= fromDate && d <= toDate;
  });
...
router.post('/teacher', async (req, res) => {
  ...
  try {
    await client.query('BEGIN');
    await client.query('DELETE FROM oqituvchilar_davomat WHERE sana=$1 AND admin_username=$2', [p.sana, p.username]);
    for (const rec of records) {
      await client.query(
        ...
        [p.sana, p.username, rec.ism, rec.fan||'', rec.status, rec.izoh||'', now, rec.dars_soat||0, rec.dars_daqiqa||0, rec.kech_minut||0]
      );
    }
    await client.query('COMMIT');
    res.json({ ok: true, saved: records.length });
...
router.get('/teacher', async (req, res) => {
  const { username, parol, sana } = req.query;
  const admin = await verifyAdmin(username, parol);
  if (!admin) return res.status(401).json({ ok: false, error: "Ruxsat yo'q" });
