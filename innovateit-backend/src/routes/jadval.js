// ─── Jadval routes ─────────────────────────────────
// GET    /api/jadval     — ro'yxat
// POST   /api/jadval     — qo'shish / yangilash (id bo'lsa update)
...
router.get('/', async (req, res) => {
  ...
  const target = !admin.isSuper ? username : (adminUsername || username);
...
router.post('/', async (req, res) => {
  ...
  if (p.id) {
    const result = await pool.query(
      ...
      [p.teacher_ism, p.teacher_familiya, p.fan||'', p.sinflar, p.kunlar, p.boshlanish||'', p.tugash||'', p.id, p.username]
    );
    ...
    await pool.query(
      ...
      [p.username, p.teacher_ism, p.teacher_familiya, p.fan||'', p.sinflar, p.kunlar, p.boshlanish||'', p.tugash||'']
    );
  }
  res.json({ ok: true });
});
